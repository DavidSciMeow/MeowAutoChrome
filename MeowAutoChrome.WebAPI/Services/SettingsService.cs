...
public class SettingsService(IProgramSettingsProvider programSettingsService, Core.Services.ScreencastServiceCore screencastService, BrowserInstanceManager browserInstances)
{
    ...
    /// <summary>
    /// 将 FPS 转换为帧间隔毫秒数。<br/>
    /// Convert FPS to frame interval milliseconds.
    /// </summary>
    private static int FpsToInterval(int fps)
        => Math.Max(16, (int)Math.Round(1000d / Math.Clamp(fps, 1, 60)));

    /// <summary>
    /// 校验程序设置模型并通过回调收集错误信息。<br/>
    /// Validate the program settings model and collect validation errors through a callback.
    /// </summary>
    /// <param name="model">待校验的设置模型。<br/>Settings model to validate.</param>
    ...
    /// <summary>
    /// 判断两个目录是否存在父子嵌套关系。<br/>
    /// Determine whether two directories are in a parent-child nested relationship.
    /// </summary>
    ...
    /// <summary>
    /// 保存程序设置并同步推流、实例启动参数等运行时状态。<br/>
    /// Save program settings and synchronize runtime state such as screencast and instance launch options.
    /// </summary>
    /// <param name="model">待保存的设置模型。<br/>Settings model to save.</param>
    /// <returns>保存结果消息。<br/>Save result message.</returns>
